...
export default function ProgramacionPage() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (mencionesLS) setMenciones(JSON.parse(mencionesLS))
      if (clientesLS) setClientes(JSON.parse(clientesLS))
      if (tiposLS) setTipos(JSON.parse(tiposLS))
    } finally {
      setLoading(false)
    }
  }
  ...
  const renovarCliente = async (clienteId: string, fechaInicio: string, fechaFin: string) => {
    try {
      // Actualizar en Firebase en lugar de localStorage
  ...
  const agregarCliente = async () => {
    if (nuevoCliente.nombre.trim() && nuevoCliente.fechaInicio && nuevoCliente.fechaFin) {
      try {
  ...
  const exportarPDF = () => {
    const registrosFiltrados = menciones.filter((mencion) => {
      ...
      const cumpleTipo = !filtroTipo || mencion.tipoMencion.includes(filtroTipo)
      const cumpleCliente = !filtroCliente || mencion.cliente.toLowerCase().includes(filtroCliente.toLowerCase())
    ...
    const contenido = `
      ...
      <html>
        ...
        <body>
          ...
          <div class="total">Total de registros: ${registrosFiltrados.length}</div>
          <table>
            <thead>
              <tr>
                <th>Fecha</th>
      ...
      <div className="min-h-screen bg-gray-50 pt-16">
        ...
        <div className="container mx-auto p-4 sm:p-6 space-y-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Panel de Programación</h1>
              <p className="text-gray-600 text-sm sm:text-base">Consulta de registros y gestión de datos maestros</p>
            </div>
          </div>

          <Tabs defaultValue="registros" className="space-y-6">
            <TabsList className="grid w-full grid-cols-3">
              <TabsTrigger value="registros" className="flex items-center space-x-2 text-sm">
                <FileText className="h-4 w-4" />
                <span className="hidden sm:inline">Registros</span>
              </TabsTrigger>
              <TabsTrigger value="clientes" className="flex items-center space-x-2 text-sm">
                <Users className="h-4 w-4" />
                <span className="hidden sm:inline">Clientes</span>
              </TabsTrigger>
              <TabsTrigger value="tipos" className="flex items-center space-x-2 text-sm">
              ...
              <div className="grid gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl sm:text-3xl font-bold text-vtv-blue">{registrosFiltrados.length}</div>
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              ...
                              <TableHead className="text-sm">Usuario</TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {registrosFiltrados.map((mencion) => (
                              <TableRow key={mencion.id}>
                                ...
                                <TableCell className="text-sm">
                                  <Badge className="bg-vtv-blue text-white text-xs">{mencion.canal}</Badge>
                                </TableCell>
                                <TableCell className="text-sm max-w-24">
                                  <div className="truncate" title={mencion.usuario}>
                                    {mencion.usuario}
                                  </div>
                                </TableCell>
                              </TableRow>
                            ...
                            <CardContent className="p-4">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  <span className="font-medium text-sm truncate pr-2 flex-1" title={mencion.cliente}>
                                    {mencion.cliente}
                                  </span>
                                  <Badge className="bg-vtv-blue text-white text-xs shrink-0">{mencion.canal}</Badge>
                                </div>
                                ...
                                <div
                                  ...
                                  title={`Registrado por: ${mencion.usuario}`}
                                >
                                  Registrado por: {mencion.usuario}
                                </div>
                              </div>
                            </CardContent>
                          </Card>
              ...
              <div className="grid gap-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription className="text-sm">Registra un cliente con período de actividad</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      <div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setNuevoCliente({ ...nuevoCliente, fechaFin: e.target.value })}
                          className="mt-1 h-10"
                        />
                      </div>
                    </div>
                    <div className="mt-4">
                      <Button
                        ...
                        disabled={!nuevoCliente.nombre.trim() || !nuevoCliente.fechaInicio || !nuevoCliente.fechaFin}
                          ...
                          <div key={cliente.id} className="flex items-center justify-between p-4 border rounded-lg">
                            ...
                            <div className="flex items-center space-x-2 shrink-0">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  confirmarAccion(`¿Estás seguro de eliminar el cliente "${cliente.nombre}"?`, () =>
              ...
              <div className="grid gap-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription className="text-sm">Registra un nuevo tipo de mención</CardDescription>
                  </CardHeader>
                  <CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid gap-3">
                      {tipos.map((tipo) => (
                        <div key={tipo.id} className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              confirmarAccion(`¿Estás seguro de eliminar el tipo "${tipo.nombre}"?`, () =>
                      ...
                      {tipos.length === 0 && (
                        <div className="text-center py-8 text-gray-500 text-sm">No hay tipos registrados</div>
                      )}
                    </div>
                  </CardContent>
                </Card>
